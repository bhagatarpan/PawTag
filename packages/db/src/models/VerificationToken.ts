...
export interface IVerificationTokenDocument extends Document {
  ...
  type: 'email_verification' | 'phone_otp' | 'password_reset' | 'login_mfa' | 'checkout_email' | 'checkout_sms';
  tokenHash: string;
  expiresAt: Date;
  usedAt?: Date;
  attempts: number;
...
const VerificationTokenSchema = new Schema<IVerificationTokenDocument>(
  {
    ...
    type: { type: String, enum: ['email_verification', 'phone_otp', 'password_reset', 'login_mfa', 'checkout_email', 'checkout_sms'], required: true, index: true },
