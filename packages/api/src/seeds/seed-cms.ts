  ...
  try {
    await session.withTransaction(async () => {
      ...
      const settings: Array<{ key: string; value: string; category: string; description?: string }> = [
        ...
        { key: 'rateLimit.global.max', value: '1000', category: 'rateLimit', description: 'Max requests per 15 minutes per IP for general API' },
        { key: 'rateLimit.auth.login.max', value: '5', category: 'rateLimit', description: 'Max login attempts per 15 minutes per IP' },
        { key: 'rateLimit.auth.register.max', value: '3', category: 'rateLimit', description: 'Max registration attempts per hour per IP' },
        { key: 'rateLimit.auth.forgotPassword.max', value: '3', category: 'rateLimit', description: 'Max password reset attempts per hour per IP' },
        { key: 'rateLimit.finder.view.max', value: '30', category: 'rateLimit', description: 'Max pet info lookups per hour per IP' },
        { key: 'rateLimit.finder.notify.max', value: '5', category: 'rateLimit', description: 'Max owner notifications per hour per IP' },
        { key: 'rateLimit.finder.location.max', value: '10', category: 'rateLimit', description: 'Max location shares per hour per IP' },
        { key: 'rateLimit.auth.mfaSend.max', value: '1', category: 'rateLimit', description: 'Max OTP send attempts per 30 seconds per IP' },
        { key: 'rateLimit.auth.mfaVerify.max', value: '5', category: 'rateLimit', description: 'Max OTP verify attempts per 15 minutes per IP' },
