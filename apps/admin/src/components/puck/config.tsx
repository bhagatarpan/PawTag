...
export const pawtagConfig: Config<PawtagComponents> = {
  components: {
      ...
      render: ({ heading, subheading, buttonText, buttonUrl, backgroundUrl }) => (
        ...
        >
          ...
          <div className="relative z-10 max-w-3xl">
            <h1 className="text-4xl font-bold mb-4">{heading || 'Your Heading'}</h1>
            <p className="text-lg mb-8 text-white/90">{subheading || 'Your subheading here'}</p>
            {buttonText && (
              <a href={buttonUrl || '#'} className="inline-block bg-white text-primary-700 font-semibold px-8 py-3 rounded-lg hover:bg-gray-100 transition">
                {buttonText}
              </a>
            )}
          </div>
      ...
      fields: {
        html: {
          ...
          render: ({ value, onChange }: { value: string; onChange: (val: string) => void }) => (
            <RichTextEditor value={value || ''} onChange={onChange} placeholder="Write your content here..." minHeight="200px" />
          ),
        },
      },
      defaultProps: {
      ...
      render: ({ heading, items }) => (
        <section className="py-12 px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {(items || []).map((item, i) => (
              <a key={i} href={item.link || '#'} className="block p-6 rounded-xl border border-gray-200 hover:shadow-lg transition">
    ...
    PricingTable: {
      fields: {
        ...
        plans: {
          type: 'array',
          label: 'Plans',
          defaultItemProps: { name: '', price: '', features: '', cta: '', ctaUrl: '', highlighted: 'false' },
          arrayFields: {
    ...
    TestimonialsSection: {
      ...
      render: ({ heading, items }) => (
        <section className="py-16 px-6 bg-gray-50 rounded-xl">
          <h2 className="text-3xl font-bold text-center mb-12">{heading || 'Testimonials'}</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            {(items || []).map((item, i) => (
          ...
          <div className="space-y-4">
            {(items || []).map((item, i) => (
              <details key={i} className="border border-gray-200 rounded-lg">
                <summary className="px-6 py-4 font-medium cursor-pointer hover:bg-gray-50">{item.question}</summary>
    ...
    StatsCounter: {
      ...
      defaultProps: { heading: 'Our Impact', stats: [] },
      render: ({ heading, stats }) => (
        <section className="py-16 px-6 bg-primary-50 rounded-xl">
          <h2 className="text-3xl font-bold text-center mb-12">{heading || 'Statistics'}</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 max-w-4xl mx-auto text-center">
            {(stats || []).map((stat, i) => (
              <div key={i}>
                <div className="text-3xl font-bold text-primary-700">{stat.value}{stat.suffix}</div>
          ...
          <section className="py-12 px-6 max-w-4xl mx-auto">
            {embedUrl ? (
              <div className="relative pb-[56.25%] rounded-xl overflow-hidden shadow-lg">
                <iframe src={embedUrl} className="absolute inset-0 w-full h-full" allowFullScreen title={caption || 'Video'} />
              </div>
            ) : (
              <div className="bg-gray-100 rounded-xl h-64 flex items-center justify-center text-gray-400">Enter a video URL</div>
            )}
    ...
    CtaBanner: {
      ...
      defaultProps: {
        ...
        subheading: 'Join thousands of pet owners keeping their pets safe.',
        buttonText: 'Sign Up Now',
        buttonUrl: '/register',
      },
      render: ({ heading, subheading, buttonText, buttonUrl }) => (
        <section className="py-16 px-6 bg-gradient-to-r from-primary-600 to-primary-800 text-white text-center rounded-xl">
          <h2 className="text-3xl font-bold mb-4">{heading}</h2>
          <p className="text-lg mb-8 text-white/90">{subheading}</p>
          {buttonText && (
            <a href={buttonUrl || '#'} className="inline-block bg-white text-primary-700 font-semibold px-8 py-3 rounded-lg hover:bg-gray-100 transition">
              {buttonText}
            </a>
    ...
    PartnersLogos: {
      ...
      defaultProps: { heading: 'Our Partners', logos: [] },
    ...
    ContactForm: {
      ...
      render: ({ heading, subtitle, businessHours, businessHoliday, formTitle, formButtonText }) => {
        ...
        <section className="py-12 px-6">
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-12">
              <h1 className="text-4xl font-bold text-gray-900 mb-4">{heading || 'Contact Us'}</h1>
