...
export default function CheckoutConfirmationStep({
  ...
  return (
    <div className="max-w-[1280px] mx-auto py-8 space-y-6">
      ...
      {/* Row 1: Order Details | Shipping Info | Auto-Renew */}
      ...
      {/* Row 2: Order Summary | Order Status Timeline */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4" style={{ animation: 'fade-in-up 0.4s ease-out 0.6s both' }}>
      ...
      {/* Row 3: Guardian Points | Gold Upsell | Refer a Friend */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4" style={{ animation: 'fade-in-up 0.4s ease-out 0.65s both' }}>
        {/* Points Earned */}
        {user && guardianTier && (
        ...
        {user && guardianTier && !isGoldMember && (
          <div className="bg-gradient-to-r from-amber-50 to-yellow-50 border border-amber-200 rounded-2xl p-5 flex items-start gap-3">
        ...
        {/* Refer a Friend */}
        {referralCode && (
          <div className="bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-200 rounded-2xl p-5 flex items-start gap-3">
      ...
      <div className="flex flex-wrap gap-3 justify-center" style={{ animation: 'fade-in-up 0.4s ease-out 0.7s both' }}>
        <Link to="/" className="flex items-center justify-center gap-2 px-4 py-2.5 bg-primary-600 text-white rounded-xl text-sm font-semibold hover:bg-primary-700 transition-all">
          <Home size={16} /> Home
        </Link>
        {confirmedInvoice && (
          <button
            onClick={() => handleInvoiceAction('view')}
            className="flex items-center justify-center gap-2 px-4 py-2.5 border border-gray-300 text-gray-700 rounded-xl text-sm font-semibold hover:bg-gray-50 transition-all"
          >
            <FileText size={16} /> View Invoice
          </button>
        )}
        <Link to="/account/orders" className="flex items-center justify-center gap-2 px-4 py-2.5 border border-gray-300 text-gray-700 rounded-xl text-sm font-semibold hover:bg-gray-50 transition-all">
          My Dashboard
        </Link>
        <Link to="/shop" className="flex items-center justify-center gap-2 px-4 py-2.5 border border-gray-300 text-gray-700 rounded-xl text-sm font-semibold hover:bg-gray-50 transition-all">
          Continue Shopping
        </Link>
      </div>
      ...
      {/* Invoice Section */}
      {confirmedInvoice && (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6" style={{ animation: 'fade-in-up 0.4s ease-out 0.75s both' }}>
