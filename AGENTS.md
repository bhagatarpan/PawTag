...
## Tag Lifecycle Phases

| Phase | Duration | Finder Status | Notification Status |
|-------|----------|---------------|---------------------|
| Active Period | 3 months (configurable per product) | Full access | Enabled |
...
- `inactive`: Tag created at fulfillment, not yet activated by customer
- `active`: Tag is fully operational (within Active Period or has membership)
- `limited`: Active Period expired, no membership, but warranty still valid
- `expired`: Warranty period expired, tag no longer functional
- `lost`: Pet is marked as lost
...
- Tags are created at **fulfillment time**, not order time
- Active Period starts from fulfillment date
- Warranty Period starts from fulfillment date
- Tag IDs are generated by the system and written to NFC during fulfillment
...
## Membership Extension Rules

- Gold membership extends up to 3 tags
- Platinum membership extends up to 10 tags
- Black membership extends unlimited tags
- Membership extends tags for 12 months from membership start date
- Tags beyond tier limit remain in 'limited' status if Active Period has expired

## Finder Behavior

- `active` tag: Full functionality, can notify owner
- `limited` tag: Show pet info, cannot notify owner
- `expired` tag: "Tag Expired" screen, no pet info

## Notification Rules

- 30-day warning before Active Period expires
- 7-day warning before Active Period expires
- 1-day warning before Active Period expires
- "Active Period Expired" notification when period ends
